<?php
    ...
    if ($_SESSION['timeout'] + 60 * 60 < time()) {
        ...
        if (isset($_GET['editar'])) {
            ...
            mysqli_query($connection, "UPDATE recaudo_comercializadores_2
            ...
            echo "<script>";
                echo "document.location.href = '../Recaudo_Comercializadores.php'";
            echo "</script>";
        } else {
            if (isset($_GET['eliminar'])) {
                mysqli_query($connection, "DELETE FROM recaudo_comercializadores_2 WHERE ID_RECAUDO = " . $_GET['eliminar']);
                echo "<script>";
                    echo "document.location.href = '../Recaudo_Comercializadores.php'";
                echo "</script>";
            } else {
                ...
                $id_usuario = $_SESSION['id_user'];
                mysqli_query($connection, "INSERT INTO recaudo_comercializadores_2 (ID_FACTURACION, FECHA_RECAUDO, FECHA_PAGO_BITACORA,
                ...
                mysqli_query($connection, "UPDATE facturacion_comercializadores_2 SET ESTADO_FACTURA = 1 WHERE ID_FACTURACION = '" . $id_facturacion . "'");
                $query_select_max_id_recaudo = mysqli_query($connection, "SELECT MAX(ID_RECAUDO) AS ID_RECAUDO "
                                                                           . "  FROM recaudo_comercializadores_2 ");
