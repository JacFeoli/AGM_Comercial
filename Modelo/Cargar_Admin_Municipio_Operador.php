<?php
    ...
    if ($_SESSION['timeout'] + 60 * 60 < time()) {
        ...
        $query_municipio = mysqli_query($connection, "SELECT MUN.NOMBRE AS NOMBRE_MUNICIPIO,
                                                             ...
                                                             MUN.ID_TABLA AS ID_TABLA, MUN.ID_OPERADOR
                                                        FROM municipios_operadores_2 MUN,
                                                             departamentos_operadores_2 DEP,
                                                             operadores_2 OP
                                                       WHERE MUN.ID_DEPARTAMENTO = DEP.ID_DEPARTAMENTO
                                                         AND MUN.ID_OPERADOR = OP.ID_OPERADOR
                                                         AND DEP.ID_OPERADOR = OP.ID_OPERADOR
                                                        $busqueda_municipio
        ...
        while ($row_municipio = mysqli_fetch_assoc($query_municipio)) {
            $table = $table . "<tr>";
                ...
                $table = $table . "<td style='vertical-align:middle;'><a href='Admin_Municipios_Operadores.php?id_municipio_editar=" . $row_municipio['ID_TABLA'] . "'><button><img src='Images/pencil_edit.png' title='Editar' width='16' height='16' /></button></a></td>";
                $table = $table . "<td style='vertical-align:middle;'><a href='Admin_Municipios_Operadores.php?id_municipio_eliminar=" . $row_municipio['ID_TABLA'] . "'><button><img src='Images/gnome_edit_delete.png' title='Eliminar' width='16' height='16'/></button></a></td>";
            $table = $table . "</tr>";
        }
