...
<html>
    <head>
        <title>AGM - Reportes Operadores</title>
        <meta charset="UTF-8" />
                        ...
                        <div class="col-lg-12">
                            <div class="content">
                                        ...
                                        <div id="rightcol" class="rightcol">
                                            ...
                                            <ul class="nav nav-pills" role="tablist">
                                                ...
                                                <!--<li role="presentation">
                                                    <a href="#reporte_operador_tab" id="tab_reporte_operador" aria-controls="reporte_operador_tab" role="tab" data-toggle="tab">Operador</a>
                                                </li>-->
                                                <li role="presentation">
								...
								<input type="hidden" id="id_ano_mensual" name="id_ano_mensual" value="" required="required" />
                                                ...
                                                <div role="tabpanel" class="tab-pane fade" id="reporte_operador_tab">
                                                    <form class="form-horizontal row-bottom-buffer row-top-buffer" id="reporte_operador" name="reporte_operador">
                                                        <div class="form-group">
								...
								<input type="hidden" id="id_operador" name="id_operador" value="" required="required" />
                                                                <input type="text" class="form-control input-text input-sm" id="operador" name="operador" placeholder="Operador" required="required" data-toggle="tooltip" readonly="readonly" title="OPERADOR" onclick="tipoDepartamento(1)" />
                                                            </div>
                                                            <label style="text-align: left; padding-top: 4px; white-space: nowrap;" class="col-xs-1 control-label row-label" for="periodo_operador">Periodo:</label>
                                                            <div class="col-xs-3">
								<input type="hidden" id="id_ano_operador" name="id_ano_operador" value="" required="required" />
                                                                <input type="hidden" id="id_mes_operador" name="id_mes_operador" value="" required="required" />
                                                                <input type="hidden" id="mes_operador" name="mes_operador" value="" required="required" />
                                                                <input type="text" class="form-control input-text input-sm" id="periodo_operador" name="periodo_operador" placeholder="Periodo" required="required" data-toggle="tooltip" readonly="readonly" title="PERIODO" onclick="periodoLiquidaciones(1)" />
                                                            </div>
                                                        </div>
                                                        <div class="form-group">
                                                            <div class="col-xs-12">
    ...
    <script>
        ...
        function resetFields(id_menu) {
            switch (id_menu) {
                case 0:
                    ...
                    $("#id_mes_operador").val("");
                    $("#mes_operador").val("");
                    $("#id_departamento_operador").val("");
                    $("#id_municipio_operador").val("");
                    $("#departamento_operador").focus();
                    break;
                case 4:
                    $("#id_ano_mensual").val("");
                    $("#id_mes_mensual").val("");
        ...
        function infoPeriodoLiquidaciones(id_consulta, id_ano, id_mes, mes) {
            switch (id_consulta) {
                case '1':
                    $("#id_ano_operador").val(id_ano);
                    $("#id_mes_operador").val(id_mes);
                    $("#mes_operador").val(mes);
                    $("#periodo_operador").val(mes + " - " + id_ano);
                    break;
                case '5':
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#tab_reporte_operador").on("shown.bs.tab", function() {
                sw = 0;
                $("#departamento_operador").focus();
            });
            ...
            $("#tab_reporte_operador").on("click", function() {
                sw = 0;
                $("#departamento_operador").focus();
            });
            ...
            $("#btn_consultar").click(function() {
                switch(sw) {
                    case 0:
                        var departamento = $("#id_departamento_operador").val();
                        var municipio = $("#id_municipio_operador").val();
                        var id_ano_operador = $("#id_ano_operador").val();
                        var id_mes_operador = $("#id_mes_operador").val();
                        var periodo_operador = $("#periodo_operador").val();
                        if (departamento.length == 0) {
                            $("#departamento_operador").focus();
                            return false;
                        }
                        if (municipio.length == 0) {
                            $("#municipio_operador").focus();
                            return false;
                        }
                        if (periodo_operador.length == 0) {
                            $("#periodo_operador").focus();
                            return false;
                        }
                        ...
                        $.ajax({
                            ...
                            data: 'sw='+sw+
                                  ...
                                  '&municipio='+municipio+
                                  '&id_ano_operador='+id_ano_operador+
                                  '&id_mes_operador='+id_mes_operador+
                                  '&periodo_operador='+periodo_operador,
                            url: 'Modelo/Resultado_Operadores.php',
    ...
    <script>
        $(document).ready(function(){
            ...
            $('input[type=text][name=periodo_operador]').tooltip({
                container: "body",
                placement: "top"
            });
