...
<html>
    <head>
        <title>AGM - Archivos Cargados Catastro</title>
        <meta charset="UTF-8" />
    ...
    <div class="modal fade" id="modalDetalleArchivoCatastro" role="dialog">
                        ...
                        <div class="col-lg-12">
                            <div class="content">
                                <div class="row">
                                    ...
                                    <div class="col-md-9">
                                        <div class="rightcol">
                                            <h1>Archivos Cargados Catastro</h1>
                                            <h2></h2>
                                            <ul class="nav nav-pills" role="tablist">
                                                <li role="presentation" class="active">
                                                    <a href="#informacion_archivos_catastro_tab" id="tab_informacion_archivos_catastro" aria-controls="informacion_archivos_catastro_tab" role="tab" data-toggle="tab">Información Archivos Cargados Catastro</a>
                                                </li>
                                            </ul>
                                            <h2></h2>
                                            <div class="tab-content">
                                                <div role="tabpanel" class="tab-pane fade in active" id="informacion_archivos_catastro_tab">
                                                    <ul class="nav nav-pills ulclass_info" role="tablist">
                                                        <?php
                                                            ...
                                                            $query_select_ano = mysqli_query($connection, "SELECT DISTINCT(ACC.ANO_FACTURA) AS ANO_FACTURA, AN.ID_ANO AS ID_ANO "
                                                                                                        . "  FROM archivos_cargados_catastro_2 ACC, anos_2 AN "
                                                            ...
                                                            if (mysqli_num_rows($query_select_ano) != 0) {
                                                                while ($row_select_ano = mysqli_fetch_assoc($query_select_ano)) {
                                                                    if ($sw == 0) {
                                                                        ...
                                                                        $query_select_count_registros = mysqli_query($connection, "SELECT ID_TABLA "
                                                                                                                                . "  FROM archivos_cargados_catastro_2 "
                                                                ...
                                                                echo "<p class='message'>No se encontraron Archivos Cargados de Catastro.</p>";
                                                            }
                                                        ?>
                                                    </ul>
                                                    <h2></h2>
                                                    <div class="tab-content">
                                                        <input class="form-control input-text input-sm" type="text" placeholder="Buscar Archivo" name="buscar_archivo_catastro_info" id="buscar_archivo_catastro_info" />
                                                        <br />
                                                        <?php
                                                            $sw = 0;
                                                            ...
                                                            while ($row_select_ano = mysqli_fetch_assoc($query_select_ano)) {
                                                                ...
                                                                <div role="tabpanel" class="tab-pane fade in active" id="informacion_<?php echo $row_select_ano['ANO_FACTURA']; ?>_tab">
                                                                    <?php
                                                                        $query_select_archivos = mysqli_query($connection, "SELECT * FROM archivos_cargados_catastro_2 "
                                                                                                                         . " WHERE ANO_FACTURA = '" . $row_select_ano['ANO_FACTURA'] . "'"
        ...
        function paginacionAnos(array_id_ano, array_pagination, array_nombre_ano) {
            ...
            for (var i=0; i<array_id_ano.length; i++) {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: "archivo=cata&sw=0&id_ano="+array_id_ano[i]+"&pagination="+array_pagination[i]+"&nombre_ano="+array_nombre_ano[i],
                        ...
                        $("#"+data[0]).twbsPagination({
                            ...
                            onPageClick: function(event, page) {
                                ...
                                $.ajax({
                                    ...
                                    dataType: "json",
                                    data: "archivo=cata&sw=0"+"&id_ano="+data[3]+"&nombre_ano="+data[2]+"&page="+page,
                                    success: function(data) {
    ...
    <script>
        $(document).ready(function() {
            $("#buscar_archivo_catastro_info").focus();
            ...
            $("#tab_informacion_archivos_catastro").on("click", function() {
                $("#buscar_archivo_catastro_info").focus();
            });
            $("#buscar_archivo_catastro_info").keypress(function(e) {
                if (e.which == 13) {
                    var busqueda_archivo_catastro;
                    if ($(this).val() == "") {
                        busqueda_archivo_catastro = " ";
                    } else {
                        busqueda_archivo_catastro = $(this).val().toUpperCase();
                    }
                    ...
                    $.ajax({
                        ...
                        url: "Modelo/Cargar_Paginacion_Archivos.php",
                        dataType: "json",
                        data: "archivo=cata&sw=1&id_ano="+id_ano+"&busqueda_archivo_catastro="+busqueda_archivo_catastro,
                        success: function(data) {
                            ...
                            $("#"+data[0]).twbsPagination({
                                ...
                                onPageClick: function(event, page) {
                                    ...
                                    $.ajax({
                                        ...
                                        url: "Modelo/Cargar_Archivos.php",
                                        dataType: "json",
                                        data: "archivo=cata&sw=1&busqueda_archivo_catastro="+data[4]+"&id_ano="+data[3]+"&nombre_ano="+data[2]+"&page="+page,
                                        success: function(data) {
                                            $("#loading-spinner-"+data[2]).css('display', 'none');
            ...
            $("#modalDetalleArchivoCatastro").on('show.bs.modal', function(e) {
                ...
                $.ajax ({
                    ...
                    dataType: "json",
                    data: "archivo=cata&detalle_id="+detalle_id,
                    success: function(data) {
